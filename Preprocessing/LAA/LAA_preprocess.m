...
bordeaux_img_raw_dir = strcat(root_filespath,'\LAA_Bordeaux');
denmark_img_raw_dir = strcat(root_filespath,'\LAA_Denmark');
...
Counts_img = struct();
Counts_img.zvdim = z_axis_variance' ;
Counts_img.zrdim = z_axis_rdim' ;
writetable(struct2table(Counts_img),'Figure_variation_forhist.csv','Delimiter',',')  
...
%writetable(struct2table(Counts_img),'Figure_variation.txt','Delimiter',',')  

%% Create 3 sets, training, val and test.
% Create directory for images and landmarks
% data_preprocessed_dir = strcat(root_filespath,'\LAA_prep_data');
